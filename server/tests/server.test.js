...
describe("POST /todos", () => {
    ...
    it("should not create todo with invalid body data", (done) => {
        request(app)
            .post("/todos")
            .set("x-auth", users[0].tokens[0].token)
            .send({}) //text and _creator properties must be provided
            .expect(400)
            .end((err, res) => {
                if (err) {
                    return done(err); //return - to stop the function execution
...
describe("GET /todos/:id", () => {
    ...
    it("should not return todo doc created by other user", (done) => {
        request(app)
            .get(`/todos/${todos[1]._id.toHexString()}`) //todos[0]._id is an ObjectID so it needs to be converted to a string to be passed into URL
            .set("x-auth", users[0].tokens[0].token)
            .expect(404)
            .expect((res) => {
...
describe("DELETE /todos/:id", () => {
    ...
    it("should not remove a todo created by other user", (done) => {
        var hexId = todos[0]._id.toHexString(); //converts form ObjectID to string
